...
CREATE TABLE album (
                       ...
                       tag_id BIGINT DEFAULT NULL,                                  -- 标签 ID（引用 tag 表中的 tag_id）
...
CREATE INDEX idx_tag_id ON album (tag_id);
...
CREATE INDEX idx_tag_visibility_uploaded_time ON album (tag_id, visibility, uploaded_time);      -- 标签+可见性+上传时间的复合索引
...
-- 3.标签表
CREATE TABLE tag (
                     tag_id BIGINT AUTO_INCREMENT PRIMARY KEY,   -- 标签 ID
                     tag_name VARCHAR(100) NOT NULL,              -- 标签名称（唯一）
                     description TEXT DEFAULT NULL,               -- 标签描述（可选）
...
CREATE TABLE orders (
                        ...
                        buyer_id BIGINT NOT NULL,                                   -- 买家用户 ID
...
CREATE TABLE wishlist (
                          ...
                          created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,         -- 创建时间（愿望单创建时间）
                          UNIQUE (user_id)                                        -- 为 user_id 创建唯一索引，确保每个用户只有一个愿望单
);

-- 8.愿望单项
CREATE TABLE wishlist_item (
                                wish_list_item_id BIGINT AUTO_INCREMENT PRIMARY KEY,  -- 愿望单项 ID
                                user_id BIGINT NOT NULL,                              -- 用户 ID（和 wish_list 表关联）
                                image_id BIGINT NOT NULL,                             -- 图片 ID
                                added_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,         -- 图片添加到愿望单的时间
...
CREATE TABLE blacklist (
                           ...
                           created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP          -- 添加到黑名单的时间
);
...
CREATE TABLE complaint (
                           ...
                           handled_at TIMESTAMP DEFAULT NULL,                      -- 处理时间（如果已处理）
                           created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP          -- 创建时间
);
...
CREATE TABLE announcement (
                              ...
                              created_by BIGINT NOT NULL,                             -- 创建公告的管理员 ID
                              created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP          -- 创建时间
);

-- 15.用户公告已读状态表（关联用户和公告）
